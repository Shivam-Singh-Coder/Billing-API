...
class GroceryShopListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = grocery_models.GroceryShopDetails
        fields = ['id', 'shop_name', 'shop_owner_name', 'shop_phone', 'shop_email', 'shop_address', 'fcreated_at', 'fupdated_at']

class GroceryShopEntrySerializer(serializers.ModelSerializer):
    ...
    def validate_shop_phone(self, value):
        ...
        if self.instance:
            if grocery_models.GroceryShopDetails.objects.filter(shop_phone=value).exclude(pk=self.instance.pk).exists():
                raise serializers.ValidationError("A shop with this phone number already exists.")
        else:
            if grocery_models.GroceryShopDetails.objects.filter(shop_phone=value).exists():
                raise serializers.ValidationError("A shop with this phone number already exists.")
        return value
    ...
    def validate_shop_email(self, value):
        ...
        if value:
            ...
            if self.instance:
                if grocery_models.GroceryShopDetails.objects.filter(shop_email=value).exclude(pk=self.instance.pk).exists():
                    raise serializers.ValidationError("A shop with this email address already exists.")
            else:
                if grocery_models.GroceryShopDetails.objects.filter(shop_email=value).exists():
                    raise serializers.ValidationError("A shop with this email address already exists.")
        return value
    ...
    def create(self, validated_data):
        """Create a new GroceryShopDetails instance."""
        return grocery_models.GroceryShopDetails.objects.create(**validated_data)
    
    def update(self, instance, validated_data):
        """Update an existing GroceryShopDetails instance."""
...
class GroceryCategoryListSerializer(serializers.ModelSerializer):
    """Serializer for listing GroceryCategory instances."""
    ...
    class Meta:
        model = grocery_models.GroceryCategory
        fields = ['id', 'category_name','category_description','fcreated_at','fupdated_at']

class GroceryCategoryEntrySerializer(serializers.ModelSerializer):
    """Serializer for creating and updating GroceryCategory instances."""
    ...
    def validate_category_name(self, value):
        ...
        if self.instance:
            if grocery_models.GroceryCategory.objects.filter(category_name=value).exclude(pk=self.instance.pk).exists():
                raise serializers.ValidationError("A category with this name already exists.")
        else:
            if grocery_models.GroceryCategory.objects.filter(category_name=value).exists():
                raise serializers.ValidationError("A category with this name already exists.")
    ...
    def create(self, validated_data):
        """Create a new GroceryCategory instance."""
        return grocery_models.GroceryCategory.objects.create(**validated_data)
    
    def update(self, instance, validated_data):
        """Update an existing GroceryCategory instance dynamically."""
        for attr, value in validated_data.items():
            setattr(instance, attr, value)
        instance.save()
        return instance
...
class GroceryStockInventoryListSerializer(serializers.ModelSerializer):
    """Serializer for listing GroceryStockInventory instances."""
    ...
    class Meta:
        model = grocery_models.GroceryStockInventory
...
class GroceryStockInventoryEntrySerializer(serializers.ModelSerializer):
    """Serializer for creating and updating GroceryStockInventory instances."""
    ...
    def validate_item_name(self, value):
        ...
        # Ensure uniqueness, excluding the current instance during updates
        if self.instance:
            if grocery_models.GroceryStockInventory.objects.filter(item_name=value).exclude(pk=self.instance.pk).exists():
                raise serializers.ValidationError("An item with this name already exists.")
        else:
            if grocery_models.GroceryStockInventory.objects.filter(item_name=value).exists():
                raise serializers.ValidationError("An item with this name already exists.")
    ...
    def create(self, validated_data):
        """Create a new GroceryStockInventory instance."""
        return grocery_models.GroceryStockInventory.objects.create(**validated_data)
    
    def update(self, instance, validated_data):
        """Update an existing GroceryStockInventory instance dynamically."""
        for attr, value in validated_data.items():
            setattr(instance, attr, value)
        instance.save()
        return instance
